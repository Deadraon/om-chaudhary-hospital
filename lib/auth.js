...
export async function getCurrentUser(request) {
  ...
  // If KV is not configured or fails, fall back to JWT-only auth
  try {
    if (process.env.KV_NAMESPACE_ID && process.env.CLOUDFLARE_ACCOUNT_ID && process.env.CLOUDFLARE_API_TOKEN) {
      const kvSession = await kvGet(token);
      if (kvSession === null) {
        // KV explicitly returned 404 — session was invalidated
        // But only block if KV is the source of truth (configured)
        // For graceful degradation we still allow JWT-validated sessions
        console.warn('KV session not found for user, allowing JWT fallback');
      }
    }
  } catch (error) {
    // If KV check fails for any reason, still allow JWT-validated session
    console.error('KV session check failed (using JWT fallback):', error.message);
  }
